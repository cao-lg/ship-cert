...
const SRC = "C:/Users/caolg/Downloads/2.船舶证书（航海家）.pdf";
const OUT_PDF = "D:/workbuddy/2026-07-16-18-55-51/2.船舶证书（航海家）-标注.pdf";
const OUT_XLSX = "D:/workbuddy/2026-07-16-18-55-51/船舶证书信息-航海家.xlsx";
...
try {
  ...
  // 幽灵过滤
  const _ghost = (r) => !(r.type && r.type.includes("-")) && !r.no && !r.issue && !r.expiry && !r.annual;
  const validRecords = records.filter((r) => !_ghost(r));
  if (validRecords.length !== records.length)
    console.log(`过滤幽灵记录: ${records.length} → ${validRecords.length}`);
  ...
  const xbuf = await buildExcelWorkbook(tpl, validRecords.length ? validRecords : records);
